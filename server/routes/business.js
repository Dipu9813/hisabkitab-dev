...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Build query - temporarily remove customer_details join until FK is properly set up
      ...
      // Manually fetch customer profile pictures for loans with customer_user_id
      const formattedLoans = await Promise.all(loans.map(async (loan) => {
        let customerProfilePic = null;
        
        // If loan has a customer_user_id, fetch their profile picture
        if (loan.customer_user_id) {
          try {
            const { data: customerData, error: customerError } = await supabaseAdmin
              .from("details")
              .select("profile_pic")
              .eq("id", loan.customer_user_id)
              .single();
            ...
            console.log("⚠️ Could not fetch customer profile for loan:", loan.id);
          }
        }
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (loanError || !loan) {
        console.log("❌ Loan not found:", loanError);
        return res.status(404).json({ error: "Loan not found" });
      }

      // Verify user is a member of the business that owns this loan
      const { data: membership, error: membershipError } = await supabaseAdmin
        .from("business_members")
        .select("business_id")
      ...
      if (membershipError || !membership) {
        console.log("❌ Access denied - user not a member of business");
        return res
          .status(403)
          .json({ error: "Access denied - not a member of this business" });
      }
...
router.post("/business/:businessId/members", authenticateToken, async (req, res) => {
  ...
  console.log("➕ Add business member request:", { businessId, userId, targetUserId, role });
  ...
  try {
      ...
      .insert([
        {
          ...
          role: role // Use the provided role (defaults to 'member')
        }
      ])
      .select()
      .single();
    ...
    console.log("✅ Member added successfully:", newMembership);
    ...
    res.json({
      ...
      member: {
        ...
        details: {
          id: targetUser.id,
          full_name: targetUser.full_name,
          ph_number: targetUser.ph_number
        }
      }
    });
