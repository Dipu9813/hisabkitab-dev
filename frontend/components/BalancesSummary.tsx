...
export default function BalancesSummary({
  ...
  const getBalanceText = (balance: number) => {
    if (balance > 0) return `You are owed रु${balance.toFixed(2)}`;
    if (balance < 0) return `You owe रु${Math.abs(balance).toFixed(2)}`;
    ...
    <div className="space-y-4">
      ...
      {activeTab === "summary" && currentUserBalance && (
        <div className="space-y-4">
          {/* Your net balance */}
          <div
            className={`p-4 rounded-lg border ${getBalanceBg(
              currentUserBalance.net_balance
            )}`}
          >
            <div className="text-center">
              ...
              >
                {currentUserBalance.net_balance === 0
                  ? "रु0.00"
                  : currentUserBalance.net_balance > 0
                  ? `+रु${currentUserBalance.net_balance.toFixed(2)}`
                  : `-रु${Math.abs(currentUserBalance.net_balance).toFixed(2)}`}
              </p>
              <p
                  ...
                  <div className="text-right">
                    ...
                    >
                      {balance.net_balance === 0
                        ? "रु0.00"
                        : balance.net_balance > 0
                        ? `+रु${balance.net_balance.toFixed(2)}`
                        : `-रु${Math.abs(balance.net_balance).toFixed(2)}`}
                    </p>
                  </div>
                </div>
              </div>
