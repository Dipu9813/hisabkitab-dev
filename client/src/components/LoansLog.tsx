...
export default function LoansLog({ token }: { token: string }) {
  ...
  useEffect(() => { fetchLoans(); fetchUsers(); }, []);  const confirmLoan = async (id: string) => {
    setError("");
    setSuccess("");
    try {
    ...
    <div className="max-w-6xl mx-auto mt-8">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h3 className="text-lg font-semibold mb-4 text-blue-600">💰 Loans I've Given</h3>
          
          {/* Pending Approvals for Lenders */}
          <div className="mb-6">
            <h4 className="font-medium mb-2 text-orange-600">⏳ Pending Payment Approvals</h4>
            <div className="space-y-2">
              ...
              {loans.filter(loan => loan.lender_id === currentUserId && loan.status === 'payment_requested').length === 0 && (
                <p className="text-gray-500 italic">No pending payment approvals</p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              {loans.filter(loan => loan.lender_id === currentUserId && ['pending', 'confirmed'].includes(loan.status)).length === 0 && (
                <p className="text-gray-500 italic">No active loans given</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h3 className="text-lg font-semibold mb-4 text-green-600">📥 Loans I've Received</h3>
          
          <div className="space-y-2">
            {loans
              .filter(loan => loan.receiver_id === currentUserId)
              .map(loan => (
                <div key={loan.id} className="border rounded p-3">
            ...
            {loans.filter(loan => loan.receiver_id === currentUserId).length === 0 && (
              <p className="text-gray-500 italic">No loans received</p>
            )}
          </div>
        </div>
      </div>
