...
export default function GroupChatInterface({
      ...
      if (!res.ok) {
        ...
        try {
          if (contentType && contentType.includes("application/json")) {
            ...
            throw new Error(
              `Server error (${res.status}): ${textResponse || res.statusText}`
            );
          }
        } catch (parseError) {
  ...
  const renderMessage = (message: Message, index: number) => {
    ...
    const senderName =
      message.details?.full_name || (isOwn ? "You" : "Unknown User");
    ...
    return (
      ...
      >
        ...
        >
          {showSender && !isOwn && (
            <div className="text-xs text-gray-500 mb-1 px-3 font-medium">
              {senderName}
            </div>
          )}
          <div
