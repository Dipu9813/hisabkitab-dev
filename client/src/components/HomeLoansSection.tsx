...
export default function HomeLoansSection({ token }: { token: string }) {
  ...
  const lendedLoans = loans.filter(loan => 
    loan.lender_id === currentUserId && loan.status === 'confirmed'
  );
  
  const borrowedLoans = loans.filter(loan => 
    loan.receiver_id === currentUserId && loan.status === 'confirmed'
  );
  ...
  // New: Payment requests that need lender approval
  const pendingPaymentApprovals = loans.filter(loan => 
    loan.lender_id === currentUserId && loan.status === 'payment_requested'
  );
  ...
  useEffect(() => { 
    ...
    window.addEventListener('notificationInteraction', handleNotificationInteraction);
    
    return () => {
      clearInterval(interval);
      window.removeEventListener('storage', handleStorageChange);
      window.removeEventListener('notificationInteraction', handleNotificationInteraction);
    };
  }, []);
  ...
  const requestPayment = async (id: string) => {
    setError("");
    try {
      const res = await fetch(`http://localhost:3000/loans/${id}/payment-request`, {
        method: "POST",
        headers: { "Authorization": `Bearer ${token}` },
      });
  ...
  const confirmPayment = async (id: string) => {
    setError("");
    try {
      const res = await fetch(`http://localhost:3000/loans/${id}/confirm-payment`, {
        method: "POST",
        headers: { "Authorization": `Bearer ${token}` },
      });
  ...
  // Helper function to render a loan row
    ...
    <tr key={loan.id} className="border-b">
      ...
      <td className="py-2 px-3">
        ...
        {loan.status === 'payment_requested' && (
          <span className="bg-blue-100 text-blue-800 px-2 py-1 rounded-full text-xs">Payment Requested</span>
        )}
      </td>
      <td className="py-2 px-3">
        {loan.status === 'pending' && loan.receiver_id === currentUserId && (
          <button 
        ...
        {loan.status === 'confirmed' && loan.receiver_id === currentUserId && (
          <button 
        ...
        {loan.status === 'payment_requested' && loan.lender_id === currentUserId && (
          <button 
  ...
  const renderLoanTable = (title: string, loanList: any[], showActions: boolean = true) => (
    <div className="mb-10">
      ...
      {loanList.length === 0 ? (
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full bg-white border rounded-lg overflow-hidden">
            ...
            <tbody>
              {loanList.map(loan => renderLoanRow(loan))}
            </tbody>
          </table>
        </div>
      )}
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4">
      ...
      {pendingPaymentApprovals.length > 0 && (
        <div className="mb-8 p-4 bg-orange-50 border-l-4 border-orange-400 rounded">
          ...
          <p className="text-orange-600 mb-4 text-sm">
            These borrowers have marked their loans as paid and are waiting for your confirmation.
          </p>
      ...
      {renderLoanTable("Money I've Lent", lendedLoans)}
      
      {/* Borrowed Loans Section */}
      {renderLoanTable("Money I've Borrowed", borrowedLoans)}<div className="mt-8 flex justify-center space-x-4">
        <Link href="/lend" className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg shadow">
          Lend Money
        </Link>        <button 
        ...
        <Link href="/loans" className="bg-gray-600 hover:bg-gray-700 text-white px-6 py-2 rounded-lg shadow">
          View All Loans
        </Link>
      </div>
...
function Countdown({ deadline }: { deadline: string }) {
  ...
  return (
    <span className={`text-sm ${timeLeft === "Expired" ? "text-red-600 font-bold" : "text-blue-600"}`}>
      {timeLeft}
    </span>
  );
}
